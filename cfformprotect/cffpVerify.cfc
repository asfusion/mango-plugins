...
">
	...
	<cffunction name="setConfig" access="public" output="false" returntype="void">
		...
		<cfset variables.Config = arguments.values />
	</cffunction>
	...
	<cffunction name="testSubmission" access="public" output="false" returntype="any">
		...
		allresults.pass = pass;
		allresults.totalPoints = totalPoints;
		allresults.TestResults = TestResults;
		return allresults;
		</cfscript>
	</cffunction>
		...
		<cfif AkismetKeyIsValid>
			<cftry>
				...
				<cfhttp url="http://#getConfig().akismetAPIKey#.rest.akismet.com/1.1/comment-check" timeout="10" method="post">
					...
					<cfif Len(getConfig().akismetFormNameField)>
	...
	<cffunction name="emailReport" access="public" output="false" returntype="string">
		...
		<!--- Here is where you might want to make some changes, to customize what happens
			...
			<!---  --->
		<cfsavecontent variable="email"><cfoutput>
				...
				<ol>
					...
					<cfif StructKeyExists(arguments.TestResults,"akismet") AND NOT arguments.TestResults.akismet.Pass>
						...
						<cfset falsePositiveURL = replace(getConfig().akismetBlogURL & "&type=ham","://","^^","all")>
						...
						<cfif getConfig().akismetFormNameField neq "">
						<cfset falsePositiveURL = falsePositiveURL&"&comment_author=#urlEncodedFormat(FormStruct[getConfig().akismetFormNameField],'utf-8')#">
						</cfif>
						<cfif getConfig().akismetFormEmailField neq "">
						<cfset falsePositiveURL = falsePositiveURL&"&comment_author_email=#urlEncodedFormat(FormStruct[getConfig().akismetFormEmailField],'utf-8')#">
						</cfif>
						<cfif getConfig().akismetFormURLField neq "">
						<cfset falsePositiveURL = falsePositiveURL&"&comment_author_url=#urlEncodedFormat(FormStruct[getConfig().akismetFormURLField],'utf-8')#">
						</cfif>
						<cfset falsePositiveURL = falsePositiveURL&"&comment_content=#urlEncodedFormat(FormStruct[getConfig().akismetFormBodyField],'utf-8')#">
					...
					<cfelseif StructKeyExists(arguments.TestResults,"akismet") AND arguments.TestResults.akismet.ValidKey AND arguments.TestResults.akismet.Pass>
						...
						<cfset missedSpamURL = replace(getConfig().akismetBlogURL & "&type=spam","://","^^","all")>
						...
						<cfif getConfig().akismetFormNameField neq "">
						<cfset missedSpamURL = missedSpamURL&"&comment_author=#urlEncodedFormat(FormStruct[getConfig().akismetFormNameField],'utf-8')#">
						</cfif>
						<cfif getConfig().akismetFormEmailField neq "">
						<cfset missedSpamURL = missedSpamURL&"&comment_author_email=#urlEncodedFormat(FormStruct[getConfig().akismetFormEmailField],'utf-8')#">
						</cfif>
						<cfif getConfig().akismetFormURLField neq "">
						<cfset missedSpamURL = missedSpamURL&"&comment_author_url=#urlEncodedFormat(FormStruct[getConfig().akismetFormURLField],'utf-8')#">
						</cfif>
						<cfset missedSpamURL = missedSpamURL&"&comment_content=#urlEncodedFormat(FormStruct[getConfig().akismetFormBodyField],'utf-8')#">
	...
	<cffunction name="logFailure" acces="private" output="false" returntype="void">
		...
		<cfif StructKeyExists(arguments.TestResults,"akismet") AND NOT arguments.TestResults.akismet.Pass>
			<!--- The next few lines build the URL to submit a false
						positive notification to Akismet if this is not spam --->
			<cfset falsePositiveURL = getConfig().akismetBlogURL />
			<cfset falsePositiveURL = falsePositiveURL&"&user_ip=#urlEncodedFormat(cgi.remote_addr,'utf-8')#">
			<cfset falsePositiveURL = falsePositiveURL&"&referrer=#urlEncodedFormat(cgi.http_referer,'utf-8')#">
			<cfif getConfig().akismetFormNameField neq "">
						<cfset falsePositiveURL = falsePositiveURL&"&comment_author=#urlEncodedFormat(FormStruct[getConfig().akismetFormNameField],'utf-8')#">
		...
		<cfelseif StructKeyExists(arguments.TestResults,"akismet") AND arguments.TestResults.akismet.ValidKey AND arguments.TestResults.akismet.Pass>
			<!--- The next few lines build the URL to submit a missed
						spam notification to Akismet --->
			<cfset missedSpamURL = getConfig().akismetBlogURL />
			<cfset missedSpamURL = missedSpamURL&"&user_ip=#urlEncodedFormat(cgi.remote_addr,'utf-8')#">
			<cfset missedSpamURL = missedSpamURL&"&referrer=#urlEncodedFormat(cgi.http_referer,'utf-8')#">
			<cfif getConfig().akismetFormNameField neq "">
			<cfset missedSpamURL = missedSpamURL&"&comment_author=#urlEncodedFormat(FormStruct[getConfig().akismetFormNameField],'utf-8')#">
